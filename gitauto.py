...
class GitAuto:
    ...
    def generate_commit_message(self, diff: str) -> Optional[str]:
        ...
        if not api_key or not provider:
            self.print_warning(
                "No AI provider or API key configured. AI commit generation disabled.")
            return None
        ...
        try:
            if provider == 'anthropic':
                ...
                self.print_info(
                    "Generating commit message with Anthropic Claude...")
...
Types: feat, fix, docs, style, refactor, test, chore, changes made, updated , new , added.
Keep it under 72 characters.
                ...
                if not OpenAI:
                    self.print_warning(
                        "OpenAI library not installed or outdated.")
                    return None
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "user",
                            "content": f"Generate a concise git commit message using conventional commits format for these changes:\n\n{diff[:3000]}"}
                    ],
                    max_tokens=100,
                    temperature=1.0,
                    n=1,
                ...
                self.print_warning(
                    "Google Gemini integration is not implemented yet.")
                return None
    ...
    def run(self):
        ...
        add_files = input(
            f"{Colors.CYAN}Files to add (. for all, or specific files): {Colors.END}").strip()
        ...
        if self.get_api_key() and self.get_provider():
            use_ai_input = input(
                f"{Colors.CYAN}Generate commit message with AI? (y/n): {Colors.END}").strip().lower()
        ...
        if use_ai:
            ...
            if diff:
                ...
                if commit_message:
                    ...
                    use_generated = input(
                        f"{Colors.CYAN}Use this message? (y/n): {Colors.END}").strip().lower()
                    if use_generated != 'y':
                        commit_message = None

        if not commit_message:
            commit_message = input(
                f"{Colors.CYAN}Enter commit message: {Colors.END}").strip()
        ...
        switch_branch = input(
            f"{Colors.CYAN}Switch branch? (leave empty to stay on '{current_branch}'): {Colors.END}").strip()
        ...
        if switch_branch:
            ...
            if success:
                ...
                create_new = input(
                    f"{Colors.YELLOW}Branch doesn't exist. Create it? (y/n): {Colors.END}").strip().lower()
                if create_new == 'y':
                    success, _, error = self.run_command(
                        ['git', 'checkout', '-b', switch_branch])
                    if success:
                        self.print_success(
                            f"Created and switched to branch: {switch_branch}")
                        current_branch = switch_branch
                    else:
        ...
        push = input(
            f"{Colors.CYAN}Push to remote? (y/n): {Colors.END}").strip().lower()

        if push == 'y':
            self.print_info(f"Pushing to origin/{current_branch}...")

            success, output, error = self.run_command(
                ['git', 'push', 'origin', current_branch], capture_output=False)
            ...
            if success:
                ...
                upstream = input(
                    f"{Colors.YELLOW}Set upstream and push? (y/n): {Colors.END}").strip().lower()
                if upstream == 'y':
                    success, _, error = self.run_command(
                        ['git', 'push', '--set-upstream', 'origin', current_branch], capture_output=False)
                    if success:
                        self.print_success(
                            f"Successfully pushed and set upstream")
                    else:
...
def main():
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == 'setup':
            ...
            print(
                "To enable AI-powered commit messages, select your AI provider and provide your API key.")
            print(
                "Supported providers: anthropic, openai, gemini (gemini not implemented)")
            ...
            if provider in ('anthropic', 'openai', 'gemini'):
                api_key = input(
                    f"{Colors.CYAN}Enter API key for {provider}: {Colors.END}").strip()
                if api_key:
                    gitauto.save_config(
                        {'provider': provider, 'api_key': api_key})
                    gitauto.print_success(
